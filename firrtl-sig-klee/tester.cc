...
const lest::test spec[] = {
  ...
  CASE("uint add operator") {
    ...
    EXPECT( a16u + a64u == UInt<65>("0xe2bd5b4ff8b3dac6") );
    EXPECT( a16u + a80u == UInt<81>("0x987426c1f7cd7d4e3438") );
    EXPECT( a16u + a128u == UInt<129>("0xe903646a697fcaa344d2b2aa95e5465b") );
    EXPECT( a64u + a128u == UInt<129>("0xe903646a697fcaa427900dfa8e978b25") );
    EXPECT( a80u + a128u == UInt<129>("0xe903646a698063176b94aa781331e497") );
  },
  ...
  CASE("uint sub operator") {
    ...
    EXPECT( a16u - a16u == UInt<17>(0) );
    EXPECT( b16u - b16u == UInt<17>(0) );
    EXPECT( a64u - a64u == UInt<65>(0) );
    EXPECT( b64u - b64u == UInt<65>(0) );
    EXPECT( a80u - a80u == UInt<81>(0) );
  ...
  CASE("uint mult operator") {
    ...
    EXPECT( a16u * a64u == UInt<80>("0xb3ca61edb3960c1d7870") );
    EXPECT( a16u * a80u == UInt<96>("0x78e2e9d381fc02c167d62b8c") );
    EXPECT( a16u * a128u == UInt<144>("0xb8c3de9998d382afe24c8a07df870209c846") );
    EXPECT( a64u * a80u == UInt<144>("0x87074e58c3b16c6e47b30e3d54d831329b50") );
    EXPECT( a64u * a128u == UInt<192>("0xce61594412cec99742e7a31b66574181f294db04d7c1d3a8") );
    EXPECT( a80u * a128u == UInt<208>("0x8ac3bc6fbcdf7dc277196b2eba91fd8527c3559e9c8f30561812") );
  },
  ...
  CASE("uint binary conversion") {
    EXPECT( a16u.to_bin_str() == "1100101011111110");
    EXPECT( a64u.to_bin_str() == "1110001010111101010110110100111111111000101100110000111111001000");
    EXPECT( a80u.to_bin_str() == "10011000011101000010011011000001111101111100110101111101010011010110100100111010");
    EXPECT( a128u.to_bin_str() == "11101001000000110110010001101010011010010111111111001010101000110100010011010010101100101010101010010101111001000111101101011101");
  },
  ...
  CASE("sint sub operator") {
    ...
    EXPECT( a64s - a16s == SInt<65>("0x71088d1c4a5bdc48") );
    EXPECT( a80s - a16s == SInt<81>("0x381c1fe6bca68758b544") );
    EXPECT( a128s - a16s == SInt<129>("0x6e0939370acc19daec06e9c13db498ba") );
    EXPECT( a16s - a64s == SInt<65>("0x18ef772e3b5a423b8") );
    EXPECT( a16s - a80s == SInt<81>("0x1c7e3e019435978a74abc") );
    EXPECT( a16s - a128s == SInt<129>("0x191f6c6c8f533e62513f9163ec24b6746") );
    EXPECT( b64s - b16s == SInt<65>("0x1defaa415d9065650") );
    EXPECT( b80s - b16s == SInt<81>("0x1efbe8ae0d38ab7f3a128") );
    EXPECT( b128s - b16s == SInt<129>("0x1beb828fdbac591dba8e38eeb433f898b") );
  },
  ...
  CASE("sint binary conversion") {
    EXPECT( a16s.to_bin_str() == "0110110110111010");
    EXPECT( a64s.to_bin_str() == "0111000100001000100011010001110001001010010111000100101000000010");
    EXPECT( a80s.to_bin_str() == "00111000000111000001111111100110101111001010011010000111010110010010001011111110");
    EXPECT( a128s.to_bin_str() == "01101110000010010011100100110111000010101100110000011001110110101110110000000110111010011100000100111101101101010000011001110100");
  }
};
